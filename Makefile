...
CFLAGS_CORE = $(CFLAGS_BASE) $(shell pkg-config --cflags cairo sqlite3)
CFLAGS_GUI = $(CFLAGS_BASE) $(shell pkg-config --cflags gtk4 cairo sqlite3)
LDFLAGS_CORE = -lglpk -lm $(shell pkg-config --libs cairo sqlite3)
...
# Core source files (shared by the GUI and the benchmark)
...
.PHONY: all gui clean install benchmark
...
# Compile core objects (non-GUI: used by the benchmark)
